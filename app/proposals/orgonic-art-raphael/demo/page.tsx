...
import {
  ...
  calculateRelevance,
  generateWhyApplies,
} from "@/data/proposals/orgonic-art-raphael";
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl p-8 max-w-sm text-center shadow-lg border border-gray-100">
          ...
          <Link
            href="/proposals/orgonic-art-raphael"
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-600 mb-8">
        Discover EU regulations that may apply to your business
      </p>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-blue-100 mb-6 max-w-md">
              Answer a few questions about your business and we&apos;ll identify
              potentially relevant EU regulations.
            </p>
            <button
              onClick={onStartScan}
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard value="0" label="Regulations Found" />
        <StatCard value="0" label="Scans Completed" />
        <StatCard value="-" label="Last Scan" />
      </div>
    </motion.div>
  );
}
...
function QuestionnaireScreen({
  ...
  toggleDataProcessing: (option: string) => void;
  onNext: () => void;
  onPrev: () => void;
}) {
  const isStepValid = () => {
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <ToggleOption
                  ...
                  onChange={(v) => updateFormData("hasEmployees", v)}
                />
                <ToggleOption
                  label="Sells Online"
                  description="Do you sell online?"
                  checked={formData.sellsOnline}
                  onChange={(v) => updateFormData("sellsOnline", v)}
                />
              </div>
            </motion.div>
          )}
            ...
            >
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {demoData.dataProcessingOptions.map((option) => (
                    <button
                      key={option.value}
                      onClick={() => toggleDataProcessing(option.value)}
                      className={`px-4 py-3 rounded-lg text-sm font-medium text-left transition-colors ${
                        formData.dataProcessing.includes(option.value)
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Specific Compliance Areas of Concern
                </label>
                <textarea
                  value={formData.complianceFocus}
                  onChange={(e) =>
                    updateFormData("complianceFocus", e.target.value)
                  }
                  rows={3}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-none"
                  placeholder="Any specific regulations or areas you're concerned about?"
                />
              </div>
            </motion.div>
          )}
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 mb-8">
        Please wait while we scan EU regulations...
      </p>
...
function ResultsScreen({
  ...
  const handleExport = () => {
    ...
    toast.className =
      "fixed bottom-4 right-4 bg-gray-900 text-white px-4 py-3 rounded-lg shadow-lg z-50 animate-pulse";
    toast.textContent = "Export feature coming soon in the full version!";
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500">
            {regulations.length} potentially relevant regulations found
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
      ...
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-6 mb-6 text-white">
        ...
        <p className="text-gray-300 leading-relaxed">
          ...
          <strong className="text-white">{formData.countries.length} EU countries</strong>
          ...
          {formData.dataProcessing.length > 0 && (
            <>
              ...
              {formData.dataProcessing
                .map((dp) => demoData.dataProcessingOptions.find((o) => o.value === dp)?.label.toLowerCase())
                .join(", ")}
              .
            </>
          )}
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-bold ${
                      reg.relevanceScore >= 70
                        ? "bg-red-100 text-red-700"
                        : reg.relevanceScore >= 40
                ...
                <div className="bg-blue-50 rounded-lg p-3">
                  <p className="text-sm text-blue-800">
                    <strong>Why this may apply:</strong> {reg.whyApplies}
                  </p>
                </div>
              </div>
            </div>
